...
def get_taxi_inbounds(x, departures_f):
    ...
    def inbound_f(times):
        return times.map(lambda time: departures_f(time + datetime.timedelta(hours=dep_offset_hours)) * (k_t_day if is_day(time) else k_t_night) / psg_per_taxi)

    sample_x = pd.date_range(new_x[0], new_x[-1], periods=24 * 3)

    return new_x, interpolate(sample_x, inbound_f(sample_x))
